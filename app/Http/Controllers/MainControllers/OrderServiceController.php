    ...
    public function store(Request $request)
    {
        $v = Validator::make(
            ...
            [
                ...
                // مرفقات متعددة:
                'images.*'      => 'nullable|image|max:4096', // 4MB لكل صورة
                'videos.*'      => 'nullable|mimetypes:video/mp4,video/quicktime,video/x-msvideo|max:51200', // 50MB لكل فيديو
            ],
            [
                'service_id_fk.required' => 'الخدمة مطلوبة.',
                'images.*.image'         => 'كل ملف في images يجب أن يكون صورة.',
            ...
            if ($request->hasFile('images')) {
                ...
                foreach ($request->file('images') as $file) {
                    ...
                    OrderServiceMedia::create([
                        ...
                        'is_primary'            => $i === 0, // أول صورة أساسية
                        'sort_order'            => $i,
                        'uploaded_by_user_id_fk' => auth()->id(),
                    ]);
                    $i++;
                }
            }
            ...
            if ($request->hasFile('videos')) {
                ...
                foreach ($request->file('videos') as $file) {
                    ...
                    OrderServiceMedia::create([
                        ...
                        'mime'                  => $file->getMimeType(),
                        'size_bytes'            => $file->getSize(),
                        'duration_seconds'      => null, // إن أردت احسبها لاحقًا
                        'is_primary'            => false,
                        'sort_order'            => $j,
                        'uploaded_by_user_id_fk' => auth()->id(),
                    ]);
        ...
        return response()->json([
            'message' => 'تم إنشاء الطلب مع المرفقات.',
            'data'    => $order->load(['media']),
        ], 201);
    }
